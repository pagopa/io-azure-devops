variable "tlscert-prod-api-io-selfcare-pagopa-it" {
  default = {
    repository = {
      organization   = "pagopa"
...
locals {
  tlscert-prod-api-io-selfcare-pagopa-it = {
    tenant_id         = data.azurerm_client_config.current.tenant_id
    subscription_id   = data.azurerm_subscription.current.subscription_id
    subscription_name = "PROD-IO"
  }
  tlscert-prod-api-io-selfcare-pagopa-it-variables = {
    KEY_VAULT_SERVICE_CONNECTION = module.PROD-IO-TLS-CERT-SERVICE-CONN.service_endpoint_name
  }
  tlscert-prod-api-io-selfcare-pagopa-it-variables_secret = {
  }
}
...
module "tlscert-prod-api-io-selfcare-pagopa-it-cert_az" {
  ...
  repository                   = var.tlscert-prod-api-io-selfcare-pagopa-it.repository
  ...
  dns_zone_resource_group              = var.tlscert-prod-api-io-selfcare-pagopa-it.pipeline.dns_zone_resource_group
  tenant_id                            = local.tlscert-prod-api-io-selfcare-pagopa-it.tenant_id
  subscription_name                    = local.tlscert-prod-api-io-selfcare-pagopa-it.subscription_name
  subscription_id                      = local.tlscert-prod-api-io-selfcare-pagopa-it.subscription_id
  ...
  variables = merge(
    ...
    local.tlscert-prod-api-io-selfcare-pagopa-it-variables,
  )

  variables_secret = merge(
    var.tlscert-prod-api-io-selfcare-pagopa-it.pipeline.variables_secret,
    local.tlscert-prod-api-io-selfcare-pagopa-it-variables_secret,
  )
  ...
  schedules = {
    ...
    branch_filter = {
      include = [var.tlscert-prod-api-io-selfcare-pagopa-it.repository.branch_name]
      exclude = []
    }
  }
}
