variable "tlscert-prod-app-backend-io-italia-it" {
  default = {
    repository = {
      organization   = "pagopa"
...
module "tlscert-prod-app-backend-io-italia-it-cert_az" {
  ...
  path                         = "${local.domain}\\${var.tlscert-prod-app-backend-io-italia-it.pipeline.path}"
  github_service_connection_id = azuredevops_serviceendpoint_github.azure-devops-github-ro.id

  dns_record_name                      = var.tlscert-prod-app-backend-io-italia-it.pipeline.dns_record_name
  dns_zone_name                        = var.tlscert-prod-app-backend-io-italia-it.pipeline.dns_zone_name
  dns_zone_resource_group              = local.tlscert-prod-app-backend-io-italia-it.dns_zone_resource_group
  tenant_id                            = local.tlscert-prod-app-backend-io-italia-it.tenant_id
  subscription_name                    = local.tlscert-prod-app-backend-io-italia-it.subscription_name
  subscription_id                      = local.tlscert-prod-app-backend-io-italia-it.subscription_id
  managed_identity_resource_group_name = local.identity_rg_name

  location                            = local.location
  credential_key_vault_name           = local.tlscert-prod-app-backend-io-italia-it.credential_key_vault_name
  credential_key_vault_resource_group = local.tlscert-prod-app-backend-io-italia-it.credential_key_vault_resource_group

  variables = merge(
    var.tlscert-prod-app-backend-io-italia-it.pipeline.variables,
    local.tlscert-prod-app-backend-io-italia-it-variables,
  )

  variables_secret = merge(
    var.tlscert-prod-app-backend-io-italia-it.pipeline.variables_secret,
    local.tlscert-prod-app-backend-io-italia-it-variables_secret,
  )
